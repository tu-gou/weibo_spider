                    ...
                    if (card_type == 9):
                        ...
                        if mblog.get('pics') != None:
                            ...
                            for _ in range(len(pic_archive)):
                                ...
                                f = open(path + 'pic' + '/' + str(pic_num) + str(imgurl[-4:]),
                                         'ab')  # 存储图片，多媒体文件需要参数b（二进制文件）
                                f.write(img.content)  # 多媒体存储content
                                f.close()

                        with open(file_blog, 'a', encoding='utf-8') as fh:
...
def user_spider(user_id):
    file_blog = path + '/' + user_id + "blog.txt"
    file_info = path + '/' + user_id + "info.txt"
    get_userInfo(user_id, file_info)
    get_weibo(user_id, file_blog)
